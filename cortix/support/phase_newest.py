...
use the GetValue() method to access the history data frame (dict(list())) via columns
(dict) and rows (list).
ALERT: The corresponding values in species and quantities are OVERRIDEN and NOT to
...
class PhaseNewest:
    ...
    def __init__(
        ...
        self.__names = list()  # ordered list of names
        ...
        self.__df["time-stamps"] = [time_stamp]

        # This is meant to be the value of species concentration; a float type
        if species is not None:
            for spc in species:
                self.__df.loc[spc.name] = [0.0]

        if quantities is not None:
            for quant in quantities:  # attention here: using the original data
                self.__df[quant.name] = [quant.value]
    ...
    def __get_time_stamps(self):
        ...
        return self.__df["time-stamps"]  # return all time stamps
    ...
    def __get_actors(self):
        ...
        return self.__names  # return all names in order

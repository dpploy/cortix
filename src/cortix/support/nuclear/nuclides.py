...
class Nuclides:
    ...
    def __init__(self, propertyDensities=pandas.DataFrame()):
        assert isinstance(propertyDensities, type(pandas.DataFrame())), "fatal."
        ...
        if attributeName == "nuclides":
            if series is not None:
                ...
                if series == "fission products":
                    nuclides = [
                        x
                        for x in nuclidesNames
                        if SERIES[ELEMENTS[x.split("-")[0].strip()].series]
                        != seriesNameMap["actinides"]
                    ]
                #
                    ...
                    collec = ["C", "N", "O", "H"]
                    nuclides = [
                        x for x in nuclides if x.split("-")[0].strip() not in collec
                    ]
                #

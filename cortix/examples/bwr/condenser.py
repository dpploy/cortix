...
class Condenser(Module):
    ...
    def __condenser(self, time, temp_in, chi_in, params):
        ...
        if chi_in == -1 and temp_in > critical_temp: #superheated vapor inlet; deprecated
            ...
            q = params['steam flowrate'] * (h_steam - h_sat) * unit.kilo
            ...
            average_ht_transfer_coeff = (high_heat_transfer_coeff + \
        ...
        if chi_in > 0:
            ...
            whatisthis = (h_mixture - h_dewpt) * unit.kilo * params['steam flowrate']
            ...
            delta_Tb = whatisthis/(params['cooling water flowrate'] * water_cp * \
                    unit.kilo)
            t_c_in = 14 + 273.15
            ...
            #nusselt number at the crtical temperature according to the
            #churchill-bernsetin correlation
            liquid_nusselt = liquid_nusselt * \
                    (1 + (liquid_reynolds / 282000)**(5/6))**(4/4.5)
            #overall convective heat transfer coefficient
            liquid_heat_transfer_coeff = liquid_nusselt * \
                    (liquid_conductivity/pipe_diameter)
            ...
            xtt = ((1 -chi_in)/chi_in)**1.8 * (steam_rho/liquid_rho) * \
                    (liquid_viscosity/steam_viscosity)**0.5
            xtt = math.sqrt(xtt)
            ...
            #determine how far the two-phase mixture gets in the condenser before
            #being fully condensed
            condensation_area = (whatisthis/(alpha_sh * lmtd))
            ...
            #7. rinse and repeat
                ...
                while abs((final_runoff_temperature_guess -
                    ...
                    lmtd = ((runoff_in - final_coolant_temperature_guess) - \
                            (final_runoff_temperature - coolant_in))/\
                            math.log((runoff_in -  \
                            final_coolant_temperature_guess)/\
                            (final_runoff_temperature - coolant_in))
                    ...
                    high_nusselt = (0.3 + 0.629 * (high_reynolds ** 0.5) * \
                            high_prandlt**(1/3))
                    high_nusselt = high_nusselt/(1+(0.4/high_prandlt)**(2/3))**(1/4)
                    high_nusselt = high_nusselt * \
                            (1 + (high_reynolds / 282000)**(5/6))**(4/4.5)
                    #overall convective heat transfer coefficient
                    high_heat_transfer_coeff = high_nusselt * \
                            (high_conductivity/pipe_diameter)
                    ...
                    low_nusselt = 0.3 + 0.629 * (low_reynolds ** 0.5) * \
                            low_prandlt**(1/3)
                    low_nusselt = low_nusselt/(1+(0.4/low_prandlt)**(2/3))**(1/4)
                    low_nusselt = low_nusselt * (1 + \
                            (low_reynolds / 282000)**(5/6))**(4/4.5)
                    #overall convective heat transfer coefficient
                    low_heat_transfer_coeff = low_nusselt * \
                            (low_conductivity/pipe_diameter)

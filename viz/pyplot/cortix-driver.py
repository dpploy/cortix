...
class CortixDriver():
 ...
 def __init__( self,
  ...
  # Logging
  self.__log = logging.getLogger('launcher-pyplot_'+str(slotId)+'.pyplot')
  self.__log.info('initializing an instance of PyPlot_'+str(slotId))
  ...
  for iVar in range(nVar):
    ...
    if iVar % 9 == 0: # if a multiple of 8 start a new dashboard
      ...
      if iVar != 0: # flush any current figure
        figName = 'pyplot_'+str(self.__slotId)+'-timeseq-dashboard-'+str(iDash)+'.png'
      ...
      text = today+': vfdalib.PyPlot_'+str(self.__slotId)+': Time-Sequence Dashboard'

...
class Phase:
    ...
    def __init__(self, time_stamp=None, time_unit=None, species=None, quantities=None):
        # TODO
        """
    ...
    def get_quantity_history(self, name):
        ...
        for quant in self.__quantities:
            if quant.name == name:
                ...
                quant_history.value = self.__phase[
                    name
    ...
    def AddQuantity(self, newQuant):
        ...
        assert newQuant.name not in list(self.__phase.columns), (
            "quantity: %r exists. Current names: %r" % (newQuant, self.__phase.columns)
        )
        ...
        col = pandas.DataFrame(
            index=list(self.__phase.index), columns=[newName], dtype=object
        )
        tmp = self.__phase
        df = tmp.join(col, how="outer")
    ...
    def AddRow(self, try_time_stamp, row_values):
        ...
        assert try_time_stamp not in self.__phase.index, (
            "already used time_stamp: %r" % (try_time_stamp)
        )
        ...
        row = pandas.DataFrame(
            index=[time_stamp], columns=list(self.__phase.columns), dtype=object
        )
    ...
    def ResetHistory(self, try_time_stamp=None, value=None):
        ...
        if value is not None:
            assert (
                ...
                or isinstance(value, npy.ndarray)
            )
    ...
    def __get_time_stamp(self, try_time_stamp=None):
        ...
        if try_time_stamp is None:
            ...
            try:  # abs(index_value - try_time_stamp) <= tolerance
                loc = self.__phase.index.get_loc(

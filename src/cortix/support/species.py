...
class Species:
    ...
    def __init__(
        ...
        if (
            self.latex_name == "no-species-latex-name"
            and self.formula_name != "no-species-formula-name"
        ):
    ...
    def update_molar_mass(self):
        ...
        self.molar_mass += (
            self.charge * const.physical_constants["electron molar mass"][0]
        )
    ...
    def ordered_atoms_list(self):
        ...
        if len(self.atoms) > 1:
            ...
            sorted_atoms = [
                a
                for (i, a) in sorted(
                    zip(multipliers_lst, atoms1), key=lambda pair: pair[0], reverse=True
                )
            ]
    ...
    def __atoms_from_formula_name(self):
        ...
        while formula_name.find("[") > 1 or formula_name.find("]") > 1:
            ...
            if i != -1 and j != -1:  # if success
                ...
                formula_name = formula_name.replace(
                    formula_name[i : j + 1], multiplicity * sub_formula_name
                )
    ...
    def __latex_name_from_formula_name(self):
        ...
        for idx, c_i in enumerate(formula_name):
            ...
            if not open_parenthesis:
                if c_i == "*":
                    assert idx != 0, (
                        "fatal: incorrect complexation on formula_name = %r"
                        % formula_name
                    )
                    if formula_name[idx - 1] == "^":
                        latex_name += r"^\bullet"  # escape \
                    else:
                        assert idx != len(formula_name) - 1, (
                            "fatal: incorrect complexation on formula_name = %r"
                            % formula_name
                        )
                        latex_name += r"\bullet"  # escape \
                ...
                if idx < len(formula_name) - 1:  # numeric
                    ...
                    if (
                        c_i.isnumeric()
                        and formula_name[idx - 1].isalpha()
                        and formula_name[idx + 1] != "("
                    ):
                        latex_name += "_{" + c_i + "}"
                    elif (
                        c_i.isnumeric()
    ...
    def __str__(self):
        ...
        return s % (
            ...
            self.molar_radioactivity_unit,
            self.molar_heat_pwr,
            self.molar_heat_pwr_unit,
...
if __name__ == "__main__":
    ...
    print(tbp_org)
    no3Minus_aqu = Species(name="NO3-", formula_name="NO3^-(a)", atoms=["N", "3*O"])
    print(no3Minus_aqu)

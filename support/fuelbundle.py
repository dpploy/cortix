...
# Internal class helpers
    ...
    def __GetFreshUMass(self):
        return float(
            self._specs.loc['U mass per assy [kg]', 1]) * 1000.0  # [g]
    ...
    def __GetFuelPinLength(self):
        return float(
            self._specs.loc['Fuel rods fuel length [in]', 1]) * 2.54  # cm
    ...
    def __GetFuelRodWallThickness(self):
        return float(
            self._specs.loc['Fuel rods wall thickness [in]', 1]) * 2.54  # cm
